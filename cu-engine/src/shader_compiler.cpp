...
ShaderCompiler *ShaderCompiler::get_singleton() { return singleton; }
...
bool get_shader_stage(const std::string &p_file_extension,
                      EShLanguage &p_out_stage) {
  const size_t pos = p_file_extension.rfind('.');
  const std::string stage =
      (pos == std::string::npos)
          ? ""
          : p_file_extension.substr(p_file_extension.rfind('.') + 1);
  if (stage == "vert") {
...
bool compile_spirv(const std::string &p_filepath, const std::string &p_out_path,
                   std::vector<unsigned int> &p_spirv) {
  ...
  std::string input_glsl((std::istreambuf_iterator<char>(file)),
  ...
  shader.setEnvInput(glslang::EShSourceGlsl, shader_stage,
                     glslang::EShClientVulkan, client_input_semantics_version);
  ...
  if (!shader.preprocess(resources, default_version, ENoProfile, false, false,
                         messages, &preprocess_glsl, includer)) {
  ...
  glslang::GlslangToSpv(*Program.getIntermediate(shader_stage), p_spirv,
                        &logger, &spv_options);
  glslang::OutputSpvBin(p_spirv, p_out_path.c_str());
  return true;
}

bool ShaderCompiler::compile_shader(const std::string &p_filepath,
                                    CompiledShaderInfo *out_info) {
  const std::string out_path =
      std::filesystem::path(get_file_path(p_filepath)) /
      std::filesystem::path(
          "spv_" + std::filesystem::path(p_filepath).filename().string());
  const bool in_file_exists =
      std::filesystem::exists(std::filesystem::path(p_filepath));
  const bool out_file_exists =
      std::filesystem::exists(std::filesystem::path(out_path));
  ...
  if (out_file_exists && in_file_exists) {
    ...
    std::filesystem::file_time_type time1 =
        std::filesystem::last_write_time(std::filesystem::path(p_filepath));
    std::filesystem::file_time_type time2 =
        std::filesystem::last_write_time(std::filesystem::path(out_path));
