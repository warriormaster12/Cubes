...
const PipelineLayoutInfo LayoutAllocator::generate_pipeline_info(VkDevice p_device, const std::vector<CompiledShaderInfo>& p_shader_infos) {
    ...
    std::unordered_map<uint32_t, std::unordered_map<size_t, VkDescriptorSetLayoutBinding>> unique_bindings;
...
void CuRenderDevice::bind_descriptor(const RenderPipeline& p_pipeline, const uint32_t p_index) {
    VkCommandBuffer cmb = frame_data[current_frame_idx].cmb;
    if (p_pipeline.pipeline == VK_NULL_HANDLE) {
        return;
    }
    ...
    if (p_index >= set_count) {
        ENGINE_WARN("Cannot bind set: {}. Not that many sets have been allocated", p_index);
        return;
    }
    VkDescriptorSet set = set_count > 1 ? p_pipeline.sets[FRAME_OVERLAP * current_frame_idx + p_index] : p_pipeline.sets[current_frame_idx] ;
    vkCmdBindDescriptorSets(cmb, VK_PIPELINE_BIND_POINT_GRAPHICS, p_pipeline.layout, p_index, 1, &set, 0, 0);
}
